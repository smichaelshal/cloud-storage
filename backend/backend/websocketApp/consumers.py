        ...
        if self.isAuth == None or self.isAuth:
            if self.isFirst:
                ...
                if self.isAuth:
                    self.user = await self.getUser(self.token)
                    self.pathSource = dictUser['pathSource']
                    print("self.pathSource_o2: ", self.pathSource)
                    self.pathDestination = dictUser['pathDestination']
                    print("self.pathDestination_o2: ", self.pathDestination)
                    self.idFile = dictUser['idFile']
                    print("self.idFile_o2: ", self.idFile)
                    self.seekFile = dictUser['seekFile']
                    self.PATH_FILE = self.GLOBAL_PATH + self.idFile
                    print("self.PATH_FILE_o2:", self.PATH_FILE)
...
class ChatConsumer(AsyncWebsocketConsumer):
    ...
    async def receive(self, text_data): 
        ...
        # print("text_data_json1: ", type(text_data_json))
        # print("text_data1: ", type(text_data))
        # print("text_data_json2: ", text_data_json)
        # print("text_data_json['type']1: ", text_data_json['type'])
