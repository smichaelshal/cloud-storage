    ...
    <div class="main">
      ...
      <Directory v-for="(directory, index) in listDirectories" :key="'d' + index" @cd-directory="changeDirectory"
       ...
       <File v-for="(file, index) in listFiles" :key="'f' + index" @open-menu-file="openMenuFile"
        :name="file.nameFile" :type="file.typeFile" :id="file.idFile" />
...
export default {
  ...
  methods:{
    ...
    const _this = this;
    const url = 'http://localhost:8000/files/api/getlistfilesdirectories/';
    let config = {
          headers: {
    ...
    const _this = this;
    const url = 'http://localhost:8000/files/api/getlistdirectories/';
    let config = {
          headers: {
              Authorization: 'Token ' + this.getToken,
              }
